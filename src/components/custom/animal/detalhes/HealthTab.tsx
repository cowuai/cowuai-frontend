...
import UpdateVaccineModal from "@/components/custom/vacinacao/UpdateVaccineModal";
...
export const HealthTab = ({animal, onUpdate}: HealthTabProps) => {
    const [vacinacoesList, setVacinacoesList] = useState(animal.vacinacoes ?? []);

    const count = vacinacoesList.length; // Usa o count do estado local
    const {accessToken} = useAuth();

    useEffect(() => {
        setVacinacoesList(animal.vacinacoes ?? []);
    }, [animal.vacinacoes]);
    ...
    const onDeleteVaccine = async (vaccineId: bigint) => {
        if (!accessToken) {
            toast.error("Usuário não autenticado.");
            return;
        }

        await apiFetch(`${process.env.NEXT_PUBLIC_API_URL}/aplicacoes-vacina/${vaccineId}`, {
            method: "DELETE",
        }, accessToken);
                            ...
                            <CardHeader className="pb-3">
                                <CardTitle className="flex items-center justify-between text-lg">
                                    ...
                                    <div className={"flex items-center gap-2"}>
                                        ...
                                        <UpdateVaccineModal vaccine={vaccine} onSavedAction={async () => await onUpdate()} />
                                        <Button
                                            variant="destructive"
                                            size="sm"
                                            className="gap-2"
                                            onClick={() => onDeleteVaccine(BigInt(vaccine.id))}
                                        >
                                            <BiTrash/>
                                        </Button>
                                    </div>
                            ...
                            <CardContent className="space-y-3">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-muted-foreground">Data de
                                                Aplicação</p>
                                    ...
                                    {vaccine.proximaDose && (
                                        <div className="flex items-start gap-2">
                                            <Calendar className="h-4 w-4 text-muted-foreground mt-0.5"/>
                                            <div>
                                                <p className="text-sm font-medium text-muted-foreground">Próxima
                                                    Dose</p>
